...
def main():
	...
	for i in range(0,len(abundantNumbers)):
		for j in range(0,len(abundantNumbers)):
			while abundantNumbers[i]+abundantNumbers[j] < 28123:
				sumTwoAbundant.add(abundantNumbers[i]+abundantNumbers[j])		
				break
	
	somme=0
	for i in range(1,28123):
		if i in sumTwoAbundant:
			pass
		else :
			somme+=i
	print(somme)
